...
(function(window, document, $, undefined) {
  if (typeof Storage === "undefined") {
    window.console.error("[Better PMs]", "localStorage not supported. Aborting load.");
    return;
  } else if (typeof CLIENT.name === "undefined") {
    window.console.error("[Better PMs]", "Client is an anonymous user. Aborting load.");
    return;
  } else {
    window.console.info("[Better PMs]", "Loading Module.")
  }
  if (!window[CHANNEL.name]) { window[CHANNEL.name] = {} }
  
  class BetterPrivateMessages {
    constructor() {
      if (localStorage.getItem(`BetterPM_PrevOpen_${CLIENT.name}`) === null) {
        localStorage.setItem(`BetterPM_PrevOpen_${CLIENT.name}`, JSON.stringify([]));  // Create empty JSON
      }
      
      this.previouslyOpen = JSON.parse(localStorage.getItem(`BetterPM_PrevOpen_${CLIENT.name}`));
      this.openCache = {};
    ...
    flushCache() {
      Object.keys(this.openCache).forEach((coresp => {
        localStorage.setItem(`BetterPM_History_${CLIENT.name}_${coresp}`, JSON.stringify(this["openCache"][coresp]));
      }));
    }
    
    deployCache(coresp) {
      if (localStorage.getItem(`BetterPM_History_${CLIENT.name}_${coresp}`) === null) { return }
    ...
    initCache(coresp) {
      if (typeof this.openCache[coresp] === "undefined") {
        this.openCache[coresp] = JSON.parse(localStorage.getItem(`BetterPM_History_${CLIENT.name}_${coresp}`))
      }
    }
    ...
    saveOpen() {
      ...
      localStorage.setItem(`BetterPM_PrevOpen_${CLIENT.name}`, JSON.stringify(currOpen));
    }
    
    newMessage(coresp, msg) {
      if (localStorage.getItem(`BetterPM_History_${CLIENT.name}_${coresp}`) === null) {
        localStorage.setItem(`BetterPM_History_${CLIENT.name}_${coresp}`, JSON.stringify([]))
      }
      this.initCache(coresp);
    ...
    startUp() {
      ...
      localStorage.setItem(`BetterPM_PrevOpen_${CLIENT.name}`, JSON.stringify([]));
      return this
    }
  }
